...
| コード | ラベル | 対象業態 |
|--------|--------|----------|
| `mfg` | 製造業 | 工場・メーカー・部品製造・産業機械・電子機器製造など |
| `rtl` | 小売(EC)業 | 実店舗小売・ECサイト・通販・コンビニ・ドラッグストアなど |
| `log` | 物流(運輸)業 | 倉庫・配送・宅配・トラック運送・海運・航空貨物など |
| `const` | 建設業 | 建設・土木・設計・施工管理・建築資材など |
| `leg` | 士業 | 弁護士・税理士・公認会計士・社会保険労務士・司法書士など |
| `food` | 飲食業 | レストラン・カフェ・居酒屋・フードデリバリー・給食など |
| `htl` | 宿泊業 | ホテル・旅館・民泊・リゾート施設など |
| `re` | 不動産業 | 不動産売買・賃貸・管理・仲介・不動産テックなど |
| `ent` | 娯楽業 | ゲーム・アニメ・音楽・映像制作・テーマパーク・スポーツ・動画配信など |
| `rep` | 修理(メンテナンス)業 | 機器修理・設備保守・点検・メンテナンスサービスなど |
...
以下の客観的・簡潔な報告調に合わせる。敬語は使用しない。

**文のトーン**
- 「〜では、〜という課題があった」「〜に対し、〜を開発・導入した」「〜により、〜を達成した」
- 主語を省略せず、誰が何をしたかを明確にする
- 定量的数値は具体値で記載し、変化前後を対比させる（例: 「廃棄率が12%以上から約2.7%に圧縮された」）

**段落構成**
- 1段落3〜5文程度。背景→課題→解決策→効果へ論理的に展開
- 専門用語は日本語で説明を加えつつ、そのまま使用可（エッジAI、ハイブリッドアーキテクチャなど）

**表現の詳細**
- 箇条書きで効果を整理する際は、効果名に **太字** を付け、コロンの後に具体説明を入れる
- 外部評価・受賞歴があれば必ず含める
- 最終段落では「現在も〜進化を続けている」など、継続的な取り組みを示す締めくくりを入れる

**禁止事項**
- 「です・ます」調の敬語表現
- 感嘆符や主観的な感情表現（「素晴らしい」「驚異的」など）
- 抽象的な説明のみ（数値や具体例なしの記述）
