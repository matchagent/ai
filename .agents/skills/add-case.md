...
**メインカテゴリ（必ずいずれかを選択）**: `画像AI` / `予測AI` / `言語AI` / `AIエージェント` / `RAG` / `音声AI`
...
**使用ルール**:
- 必ず上記6つのメインカテゴリから**1つ以上**を選択すること
- 詳細タグは必要に応じて追加可能（メインカテゴリと同時に使用可）
- 該当するタグがない場合は空配列 `[]` ではなく、該当するメインカテゴリのみを設定すること
